...
banner = Image.open('C:\\Users\\emil\\Desktop\\using-machine-learning-on-datasets-with-streamlit-main\\banner.jpg')
logo = Image.open('C:\\Users\\emil\\Desktop\\using-machine-learning-on-datasets-with-streamlit-main\\logo.png')
...
if menu == 'Homepage':
    ...
    data = st.selectbox('Select Dataset', ['Water Portability', 'Loan Prediction','Income','Diabetes','Creditcard default'])
    ...
    if data == 'Loan Prediction':
        ...
        st.warning('''This dataset contains demographic and employment data used for analyzing income levels.
                   It includes various indicators such as age, type of employer, education level, marital status,
                    race, gender, weekly working hours, and income class.The dataset aims to predict the likelihood
                    of an individual's income level based on these parameters. ''')
        st.header('Dataset')
        df = pd.read_csv('C:/Users/emil/Desktop/using-machine-learning-on-datasets-with-streamlit-main/hpt_small.csv')
        st.dataframe(df)
    elif data =="Diabetes":
        st.header('Our problem')

        st.warning('''This dataset contains medical data related
                   to diabetes diagnosis among pima indians.it includes
                   various health indicators such as the number of pregnancies,glucose concentration
                   blood pressure, skin thickness,insulin level,body mass index
                   diabetes pedigree function age, and the diabetes outcome.
                   this dataset is used to predict the likelihood of diabetes on these
                   medical parameteres''')
        st.header('Dataset')
        df = pd.read_csv('C:/Users/emil/Desktop/using-machine-learning-on-datasets-with-streamlit-main/diabetes.csv')
        st.dataframe(df)
    elif data == "Creditcard default":
        st.header('Our problem')

        st.warning('''This dataset contains information on credit card
                    default payments based on data from a Taiwanese bank.
                   It includes various attributes such as credit limit,
                    gender, education level, marital status, and age.
                   Additionally, the dataset captures payment status and amounts over six months.
                    The attribute indicating whether the customer defaulted on their next month's
                    payment provides insights into factors influencing credit card default risk.''')
        st.header('Dataset')
        df = pd.read_csv('C:/Users/emil/Desktop/using-machine-learning-on-datasets-with-streamlit-main/CreditCardDefault.csv')
        st.dataframe(df)    
    ...
    def describe(df):
        ...
        if c2.button('Data Preprocessing'):
            ...
            if cat_array.size > 0:
                if cat_met == 'Mode':
                    ...
                    df[cat_array] = df[cat_array].fillna(method='backfill')

                else:
                    df[cat_array] = df[cat_array].fillna(method='ffill')
        ...
        # The corr() method in pandas is used to calculate the correlation between numeric columns, 
        # and it cannot handle non-numeric (categorical) columns. So, we can do it for numerical columns.
        # (if you remember there is a mistake for Loan dataset, and now i handled it . So code will work for both dataset properly.)

        numeric_df = df.select_dtypes(include='number')
        heatmap = px.imshow(numeric_df.corr())
    ...
    st.header('EDA')
    dataset = st.selectbox('Select Dataset', ['Water Portability', 'Loan Prediction','Income','Diabetes', 'Credit card default'])
    st.markdown(f'Selected: {dataset} dataset')
    
    if dataset == 'Loan Prediction':
        df = pd.read_csv('loan_pred (1).csv')
        describe(df)
    elif dataset =="Water Portability":
        df = pd.read_csv('water_potability (1).csv')
        describe(df)
    elif dataset == "Income":
        df = pd.read_csv('C:/Users/emil/Desktop/using-machine-learning-on-datasets-with-streamlit-main/hpt_small.csv')
        ...
        df = pd.read_csv('C:/Users/emil/Desktop/using-machine-learning-on-datasets-with-streamlit-main/diabetes.csv')
        describe(df)
    elif dataset =="Credit card default":
        df = pd.read_csv('C:/Users/emil/Desktop/using-machine-learning-on-datasets-with-streamlit-main/CreditCardDefault.csv')
        describe(df)
